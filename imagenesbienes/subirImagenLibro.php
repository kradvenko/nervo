    ...
    if(isset($_FILES["imgInp"]["type"]))
    {
        ...
        if ((($_FILES["imgInp"]["type"] == "image/png") || ($_FILES["imgInp"]["type"] == "image/jpg") || ($_FILES["imgInp"]["type"] == "image/jpeg"))) {
                ...
                else
                {
                    ...
                    $targetPath = "libros/" . $_POST["idFichaLibro"] . "/" . $_FILES['imgInp']['name']; // Target path where file is to be stored
                    if (!file_exists("libros/" . $_POST["idFichaLibro"] . "/")) {
                        mkdir("libros/" . $_POST["idFichaLibro"] . "/", 0777, true);
                    }
                    move_uploaded_file($sourcePath, $targetPath) ; // Moving Uploaded file
                    $thumbPath = "thumbs/libros/" . $_POST["idFichaLibro"] ."_" . $_FILES['imgInp']['name'];
